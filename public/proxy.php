...
if (isset($_SERVER['_ENV']) && $_SERVER['_ENV'] == "production") {
    define('API_URL',               'http://api.ruinitforme.com/');
    define('CURRENT_PROXY_URL',     'http://www.ruinitforme.com/proxy.php');
    define('CURRENT_PROXY_URL_TWO', 'http://ruinitforme.com/proxy.php');
} else {
    error_reporting(E_ALL);
    ini_set('display_errors', '1');
    define('API_URL', 'local.api.ruinitforme.com/');
    define('CURRENT_PROXY_URL', 'http://local.backbone.ruinitforme.com/proxy.php');
    define('CURRENT_PROXY_URL_TWO', 'http://local.backbone.ruinitforme.com/proxy.php');
}
    ...
    public function indexAction () {
        ...
        # we are logging in, so auth
            ...
            if ($sessionId) {
                ...
                echo json_encode(array('user' => $login->user, 'permissions' => $login->permissions, 'login' => true));
                exit;
            } else {
        ...
        # signing up
            ...
            /* now fetch the user */
            $request = new OpenSource_Rest(API_URL.'system/session', 'GET');
            $request->setUsername('aaron.lote@hivetracking.com');
